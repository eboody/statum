...
fn get_field_info(input: &DeriveInput) -> (Vec<&syn::Ident>, Vec<&syn::Type>) {
    match &input.data {
        syn::Data::Struct(s) => match &s.fields {
            ...
            _ => {
                ::core::panicking::panic_fmt(format_args!("Only named fields are supported"));
            }
        },
        _ => {
...
pub fn state(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let variants: Vec<VariantInfo> = match &input.data {
        Data::Enum(data_enum) => data_enum
            ...
            .collect(),
        _ => {
            ::core::panicking::panic_fmt(format_args!("#[state] can only be used on enums"));
        }
    };
    ...
    if !replicate_string.is_empty() {
        ...
        state_derives_ref
            .lock()
            .unwrap()
            .push_str(&replicate_string);
        {
            ::std::io::_print(format_args!("replicate_derives: {0}\n", replicate_derives));
        };
    }
...
fn extract_state_trait(input: &DeriveInput) -> syn::Ident {
    ...
    {
        ::core::panicking::panic_fmt(format_args!("Type parameter must have a trait bound"));
    };
}
fn analyze_user_derives(
    ...
    for attr in attrs {
        if attr.path().is_ident("derive") {
            if let Ok(paths) =
                attr.parse_args_with(Punctuated::<Path, ::syn::token::Comma>::parse_terminated)
            {
                for path in paths {
...
pub fn model(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let try_methods = variants.iter().map(|variant| {
        let variant_ident = match ::quote::__private::IdentFragmentAdapter(&variant.name) {
            arg => ::quote::__private::mk_ident(
        ...
        let try_method_name =
            match ::quote::__private::IdentFragmentAdapter(&to_snake_case(&variant.name)) {
                arg => ::quote::__private::mk_ident(
                    &::alloc::__export::must_use({
                        let res = ::alloc::fmt::format(format_args!("try_to_{0}", arg));
                        res
                    }),
                    ::quote::__private::Option::None.or(arg.span()),
                ),
...
pub fn validators(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let wrapper_enum_ident = match ::quote::__private::IdentFragmentAdapter(&machine_ident) {
        arg => ::quote::__private::mk_ident(
            &::alloc::__export::must_use({
    ...
    {
        ::std::io::_print(format_args!("state_derives: {0}\n", {
            let mut _s = ::quote::__private::TokenStream::new();
            ...
            ::quote::__private::push_group(&mut _s, ::quote::__private::Delimiter::Brace, {
                ...
                ::quote::__private::push_group(&mut _s, ::quote::__private::Delimiter::Brace, {
                    let mut _s = ::quote::__private::TokenStream::new();
                    {
                        ...
                        let has_iter = ::quote::__private::ThereIsNoIteratorInRepetition;
                        #[allow(unused_mut)]
                        let (mut wrapper_variants_match_arms, i) =
                            wrapper_variants_match_arms.quote_into_iter();
                        let has_iter = has_iter | i;
                        let _: ::quote::__private::HasIterator = has_iter;
                        while true {
                            let wrapper_variants_match_arms =
                                match wrapper_variants_match_arms.next() {
                                    Some(_x) => ::quote::__private::RepInterp(_x),
                                    None => break,
                                };
                            if _i > 0 {
                            ...
                            _i += 1;
                            ::quote::ToTokens::to_tokens(&wrapper_variants_match_arms, &mut _s);
                        }
                    }
                    _s
                });
    ...
    let field_idents = fields
        .iter()
        .map(
            |(name, _type)| match ::quote::__private::IdentFragmentAdapter(&name) {
                arg => ::quote::__private::mk_ident(
                    &::alloc::__export::must_use({
                        let res = ::alloc::fmt::format(format_args!("{0}", arg));
                        res
            ...
            if let syn::ImplItem::Fn(method) = item {
                if method.sig.ident.to_string().starts_with("is_") {
                    ...
                    for (field_name, field_type) in &fields {
                        ...
                        updated_inputs.push(syn::FnArg::Typed(::syn::__private::parse_quote({
                            let mut _s = ::quote::__private::TokenStream::new();
                            ...
                            ::quote::__private::Delimiter::Parenthesis,
                            {
                                let mut _s = ::quote::__private::TokenStream::new();
                                {
                                    ...
                                    let has_iter =
                                        ::quote::__private::ThereIsNoIteratorInRepetition;
                                    #[allow(unused_mut)]
                                    let (mut updated_inputs, i) = updated_inputs.quote_into_iter();
                                    let has_iter = has_iter | i;
                                    let _: ::quote::__private::HasIterator = has_iter;
                                    while true {
    ...
    let modified_impl_block = {
        ...
        ::quote::__private::push_group(&mut _s, ::quote::__private::Delimiter::Brace, {
            let mut _s = ::quote::__private::TokenStream::new();
            {
                ...
                let has_iter = ::quote::__private::ThereIsNoIteratorInRepetition;
                #[allow(unused_mut)]
                let (mut modified_impl_items, i) = modified_impl_items.quote_into_iter();
                let has_iter = has_iter | i;
                let _: ::quote::__private::HasIterator = has_iter;
                while true {
...
fn build_to_machine_fn(
    ...
    for variant_info in enum_variants {
        ...
        let is_method_ident = match ::quote::__private::IdentFragmentAdapter(&variant_snake) {
            arg => ::quote::__private::mk_ident(
                &::alloc::__export::must_use({
                    let res = ::alloc::fmt::format(format_args!("is_{0}", arg));
                    res
        ...
        if let Some(f) = user_fn {
            ...
            {
                ::std::io::_print(format_args!("No user_fn for variant \'{0}\'\n", variant));
            };
            checks.push(
                syn::Error::new(
                ...
                if seg.ident == "Result" {
                    if let PathArguments::AngleBracketed(args) = &seg.arguments {
                        ...
                        if args.len() == 1 {
                            ...
                            {
                                ::core::panicking::panic_fmt(format_args!(
                                    "Expected 1 or 2 arguments in Result<_, _>"
                                ));
                            };
                        }
                    }
