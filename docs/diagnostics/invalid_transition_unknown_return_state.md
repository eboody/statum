# Transition Unknown Return State

Status: first-party diagnostic

Source fixture: `../../statum-macros/tests/ui/invalid_transition_unknown_return_state.rs`
Compiler-output fixture: `../../statum-macros/tests/ui/invalid_transition_unknown_return_state.stderr`
...
```text
error: Error: transition method `to_ghost` returns state `Ghost`, but `Ghost` is not a variant of `#[state]` enum `State`.
  ...
  --> tests/ui/invalid_transition_unknown_return_state.rs:27:26
   |
27 |     fn to_ghost(self) -> Machine<Ghost> {
   |                          ^^^^^^^
```
